...
// Choose source based on connectivity
                ...
                repository.getNotifications(false, source, PAGE_SIZE)
                    .fold( // Pass PAGE_SIZE to repository
...
// If no new notifications are returned, we are on the last page
    ...
    /**
     * Refreshes the unread notification count.
     * This method is designed to be resilient to cancellation.
     */
    /**
     * Refreshes the unread notification count.
     * This method is designed to be resilient to cancellation.
    ...
    fun refreshUnreadCount() {
        ...
        loadUnreadCountJob = viewModelScope.launch {
            try {
                ...
                val count = repository.getUnreadNotificationCountSafe()
                if (count >= 0) {
                    _unreadCount.value = count
                }
            } catch (e: kotlinx.coroutines.CancellationException) {
                // Silently ignore cancellations
                Log.d("NotificationViewModel", "Unread count refresh was cancelled")
            } catch (e: Exception) {
                Log.e("NotificationViewModel", "Exception in unread count refresh", e)
            }
        }
    }
    /**
