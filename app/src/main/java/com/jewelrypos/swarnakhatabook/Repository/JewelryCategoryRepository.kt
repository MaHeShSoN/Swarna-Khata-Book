...
class JewelryCategoryRepository(
    ...
    private fun userCategoriesCollection() = firestore
        .collection("users")
    ...
    suspend fun getCategoriesByMetalType(metalType: String): Result<List<JewelryCategory>> =
        suspendCoroutine { continuation ->
            try {
                userCategoriesCollection()
                    .whereEqualTo("metalType", metalType)
                    .get()
        ...
        try {
            ...
            userCategoriesCollection()
                ...
                .addOnSuccessListener { documents ->
                    ...
                    // ... create defaultCategories as before, but do not set createdBy (it will be set below) ...
    ...
    // Silver Jewelry Categories
                    ...
                    defaultCategories.forEach { category ->
                        val docRef = userCategoriesCollection().document()
                        batch.set(docRef, category.copy(createdBy = userId))
                    }
                    batch.commit()
